...
void ExecuteJump(uint8_t JumpType,float JumpAngle)
{
    if (JumpType == Standard_Jump)//标准跳（对绝大多数地面都有较好的适应性，兼具高度和远度）
    {
        ...
        ChangeGainOfPID(35.0f,0.1f,0, 0);//使用刚度小，阻尼大的增益0
...
//        gpstate = 1;
    ...
    else if(JumpType == High_Jump)//简单原地跳个高（任何地面都行）
    {
        ...
        ChangeGainOfPID(32.0f,0.1f,0,0);// 使用高刚度和低阻尼执行跳转
//        PID_Set_KP_KI_KD(&AngleLoop[3],32.0f,0,0.45f);
//        PID_Set_KP_KI_KD(&AngleLoop[4],32.0f,0,0.45f);
//        PID_Set_KP_KI_KD(&AngleLoop[7],32.0f,0,0.45f);
//        PID_Set_KP_KI_KD(&AngleLoop[8],32.0f,0,0.45f);
        SetPolarPositionAll_Delay(JumpAngle,jump_extension,launch_time);
        //飞翔过程（也即降落过程）中的姿态（核心），持续时间为fall_time
        AllLegsSpeedLimit(SpeedMode_VERYFAST);
        ChangeGainOfPID(5.0f,0.1f,0,0);//使用低刚度和大量的阻尼来处理下降
        SetPolarPositionAll_Delay(-40,jump_flylegheight,fall_time);
        //脚用力准备站起来
        ChangeGainOfPID(6,0.1f,0.0f,0.0f);//使用低刚度和大量的阻尼来处理下降
    ...
    else if(JumpType == Far_Jump)//简单跳个远（要求地面摩擦较大）
    {
        ...
        ChangeGainOfPID(30.0f,0.15f,0.0f,0.0f);
        SetPolarPositionAll_Delay(JumpAngle,jump_extension,launch_time);
        //飞翔过程（也即降落过程）中的姿态（核心），持续时间为fall_time
        AllLegsSpeedLimit(SpeedMode_EXTREME);
        ChangeGainOfPID(9,0.1f,0.0f,0.0f);//使用低刚度和大量的阻尼来处理下降
